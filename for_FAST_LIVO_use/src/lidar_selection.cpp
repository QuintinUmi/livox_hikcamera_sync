...
void LidarSelector::detect(cv::Mat img, PointCloudXYZI::Ptr pg, double scale_img) 
{
    img_rgb = img.clone();
    if(width!=img.cols || height!=img.rows)
    {
        // std::cout<<"Resize the img scale !!!"<<std::endl;
        cv::resize(img,img,cv::Size(img.cols*scale_img,img.rows*scale_img),0,0,CV_INTER_LINEAR);
    }
